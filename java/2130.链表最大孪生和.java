...
class Solution {
    public int pairSum(ListNode head) {
        ...
        ListNode node = reverseFirstHalf(head, firstHalfEnd);
        ListNode oviparity = secondHalfStart;
        while (node != null) {
            int sum = node.val + oviparity.val;
            maxSum = Math.max(maxSum, sum);
            node = node.next;
            oviparity = oviparity.next;
        }

        return maxSum;
    }

    private ListNode reverseFirstHalf(ListNode head, ListNode firstHalfEnd) {
