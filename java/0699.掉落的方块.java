...
class Solution {
    public List<Integer> fallingSquares(int[][] positions) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            int height = sideLength, right = left + sideLength - 1;

            for (int j = 0; j < i; j++) {
                int preLeft = positions[j][0];
                int preRight = preLeft + positions[j][1] - 1;

                if (preLeft <= right && preRight >= left)
                    height = Math.max(height, result.get(j) + sideLength);
            }
