 ...
 * 是一个升序排列的列表，其中 releaseTimes[i] 表示松开第 i 个键的时间。字符串和数组的 下标都从 0 开始 。第 0 个键在时间为 0
 * 时被按下，接下来每个键都 恰好 在前一个键松开时被按下。
 * 
 * 测试人员想要找出按键 持续时间最长 的键。第 i^ 次按键的持续时间为 releaseTimes[i] - releaseTimes[i - 1] ，第
 * 0 次按键的持续时间为 releaseTimes[0] 。
 * 
...
class Solution {
    public char slowestKey(int[] releaseTimes, String keysPressed) {
        ...
        for (int i = 1; i < releaseTimes.length; i++) {
            ...
            if (duration > maxPressedTime) {
                maxPressedTime = duration;
                c = keysPressed.charAt(i);
            } else if (duration == maxPressedTime)
