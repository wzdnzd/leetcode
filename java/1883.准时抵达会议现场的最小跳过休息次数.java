 ...
 *
 * 给你一个整数 hoursBefore ，表示你要前往会议所剩下的可用小时数。要想成功抵达会议现场，你必须途经 n 条道路。道路的长度用一个长度为 n
 * 的整数数组 dist 表示，其中 dist[i] 表示第 i 条道路的长度（单位：千米）。另给你一个整数 speed
 ...
 * 
 * 
 * 例如，如果你通过一条道路用去 1.4 小时，那你必须停下来等待，到 2 小时才可以继续通过下一条道路。如果通过一条道路恰好用去 2
 * 小时，就无需等待，可以直接继续。
 * 
 * 
 * 然而，为了能准时到达，你可以选择 跳过
 ...
 * 小时才能抵达会议现场。
 * 可以跳过第 1 次和第 3 次休息时间，共用 ((7/2 + 0) + (3/2 + 0)) + ((5/2 + 0) + (5/2)) = 10
 * 小时抵达会议现场。
 * 
 * 
 * 示例 3：
...
class Solution {
    ...
    // 无穷大值
    private static final double INFINITY = 1e20;
    ...
    public int minSkips(int[] dist, int speed, int hoursBefore) {
        ...
        for (int i = 1; i <= n; i++) {
            for (int j = 0; j <= i; j++) {
                double cost = (double) dist[i - 1] / speed;

                if (j != i)
                    dp[i][j] = Math.min(dp[i][j], Math.ceil(dp[i - 1][j] + cost - EPSILON));
