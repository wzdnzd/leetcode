...
class Solution {
    public int minimumRefill(int[] plants, int capacityA, int capacityB) {
        int n = plants.length, count = 0;
        int left = 0, right = n - 1;
        int remainAlice = capacityA, remainBob = capacityB;

        while (left < right) {
            if (remainAlice < plants[left]) {
                ++count;
                remainAlice = capacityA;
            }

            remainAlice -= plants[left++];

            if (remainBob < plants[right]) {
                ++count;
                remainBob = capacityB;
            }

            remainBob -= plants[right--];
        }

        if (left == right) {
            int maxRemain = remainAlice >= remainBob ? remainAlice : remainBob;
            if (maxRemain < plants[left])
                ++count;
        }
