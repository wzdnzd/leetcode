 ...
 * 
 * 你可以随时停下摩天轮，即便是 在服务所有游客之前 。如果你决定停止运营摩天轮，为了保证所有游客安全着陆，将免费进行所有后续轮转 。注意，如果有超过 4
 * 位游客在等摩天轮，那么只有 4 位游客可以登上摩天轮，其余的需要等待 下一次轮转 。
 * 
...
class Solution {
    public int minOperationsMaxProfit(int[] customers, int boardingCost, int runningCost) {
        int count = -1, profit = 0, maxProfit = 0;
        int wait = 0, current = 0;

        while (wait > 0 || current < customers.length) {
            wait += current < customers.length ? customers[current] : 0;
            int aboard = Math.min(4, wait);
            wait -= aboard;
            ++current;
