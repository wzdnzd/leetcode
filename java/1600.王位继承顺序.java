...
class ThroneInheritance {
    ...
    public void birth(String parentName, String childName) {
        this.graph.computeIfAbsent(parentName, k -> new ArrayList<>()).add(childName);
    }
    ...
    public List<String> getInheritanceOrder() {
        ...
        preorder(this.king, result);
        return result;
    }

    private void preorder(String name, List<String> result) {
        if (!this.dead.contains(name))
            result.add(name);

        for (String child : this.graph.getOrDefault(name, List.of()))
