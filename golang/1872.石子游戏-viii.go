 ...
 * 分数差。
 *
 * 给你一个长度为 n 的整数数组 stones ，其中 stones[i] 是 从左边起 第 i 个石子的价值。请你返回在双方都采用 最优
 ...
 * 输出：5
 * 解释：
 * - Alice 移除最左边的 4 个石子，得分增加 (-1) + 2 + (-3) + 4 = 2 ，并且将一个价值为 2
 * 的石子放在最左边。stones = [2,-5] 。
 ...
 * 输出：13
 * 解释：
 * - Alice 移除所有石子，得分增加 7 + (-6) + 5 + 10 + 5 + (-2) + (-6) = 13 ，并且将一个价值为 13
 * 的石子放在最左边。stones = [13] 。
 * 两者分数之差为 13 - 0 = 13 。
 *
 *
 ...
 * 输出：-22
 * 解释：
 * - Alice 只有一种操作，就是移除所有石子。得分增加 (-10) + (-12) = -22 ，并且将一个价值为 -22
 * 的石子放在最左边。stones = [-22] 。
 * 两者分数之差为 (-22) - 0 = -22 。
 *
 *
