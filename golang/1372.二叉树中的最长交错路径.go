...
func longestZigZag(root *TreeNode) int {
	...
	dfs = func(root *TreeNode, isLeft bool, depth int) int {
		if root == nil {
			return depth
		}

		if isLeft {
			return max(dfs(root.Right, false, depth+1), dfs(root.Left, true, 0))
		}

		return max(dfs(root.Left, true, depth+1), dfs(root.Right, false, 0))
	}
