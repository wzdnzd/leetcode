...
class Solution:
    def minTaps(self, n: int, ranges: List[int]) -> int:
        spaces = [[max(i - x, 0), i + x] for i, x in enumerate(ranges)]
        spaces.sort(key=lambda x: [x[0], -x[1]])
        ans, index, right = 0, 0, 0

        while index < len(spaces) and right < n:
            start, end = spaces[index]
            if start > right:
                return -1
            ...
            if start < right:
                while index < n and spaces[index + 1][0] <= right:
                    index += 1
                    end = max(end, spaces[index][1])
